...
def stap0():
    global literprice
    global liter
    stapoo = input('Bent u 1) particulier of 2) zakelijk?')
    if stapoo == '1':
        stap1()
    elif stapoo == '2':
        liter = int(input('Hoeveel liter wilt u kopen?($9.80 per liter)\n excluding BTW'))
        literprice = liter * 9.80
        literprice = literprice * 0.09
        print (literprice)
        quit()
    else:
